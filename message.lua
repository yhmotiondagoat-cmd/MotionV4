	...
	if not shared.vapereload then
		...
		if vape.Categories.Main.Options['GUI bind indicator'].Enabled then
			if getgenv().catrole == 'HWID MISMATCH' then
				vape:CreateNotification('MOTION V4', 'HWID MISMATCH, Go to the script panel to reset hwid', 25, 'alert')
				getgenv().catrole = ''
				task.wait(0.1)
			end
			if vape.Place ~= 6872274481 and not license.Closet then
				-- Discord redirect disabled
			end
...
local function motionv4EnsureFolder(path)
	if isfolder and not isfolder(path) then
		makefolder(path)
	end
end
motionv4EnsureFolder('catrewrite')
motionv4EnsureFolder('catrewrite/guis')
writefile('catrewrite/guis/new.lua', [=[
local license = ... or {}
local mainapi = {
	Categories = {},
	GUIColor = {
...
local getcustomassets = {
	...
	['catrewrite/assets/new/camo_print.png'] = 'rbxassetid://93318909297877',
...
local function addCamo(parent)
	local camo = Instance.new('ImageLabel')
	camo.Name = 'CamoOverlay'
	camo.Size = UDim2.new(1, 0, 1, -35)
	camo.Position = UDim2.fromOffset(0, 35)
	camo.BackgroundTransparency = 1
	camo.Image = getcustomasset('catrewrite/assets/new/camo_print.png')
	camo.ImageTransparency = 0.25
...
local function createDownloader(text)
	if mainapi.Loaded ~= true then
		...
		pcall(function()
			downloader.Text = 'Downloading Motionrewrite'
		end)
	end
end
...
function mainapi:CreateGUI()
	...
	logo.Name = 'VapeLogo'
	logo.Size = UDim2.fromOffset(150, 30)
	logo.Position = UDim2.fromOffset(9, 3)
	logo.BackgroundTransparency = 1
	logo.Parent = window
        local logoImage = Instance.new("ImageLabel")
        logoImage.Name = "MotionLogo"
        logoImage.BackgroundTransparency = 1
        logoImage.Size = UDim2.fromOffset(150, 30)
        logoImage.Position = UDim2.fromOffset(0, 0)

-- PUT YOUR IMAGE ID HERE
logoImage.Image = "rbxassetid://75644866213300"
...
logoImage.Parent = logo
	...
	function categoryapi:CreateButton(categorysettings)
		...
		function optionapi:Toggle()
			...
			if self.Enabled and categorysettings.OpenWindow then
				categorysettings.OpenWindow()
			end
		end
	...
	discordbutton.MouseButton1Click:Connect(function()
		...
		task.spawn(function()
			tooltip.Text = 'Copied!'
			setclipboard('https://discord.gg/99c49d34')
		end)
	end)
...
function mainapi:CreateCategory(categorysettings)
	...
	function categoryapi:Expand(expanded)
		self.Expanded = expanded == nil and not self.Expanded or expanded
	...
	window.MouseButton1Click:Connect(function()
		if inputService:GetMouseLocation().Y < window.AbsolutePosition.Y + 41 then
			categoryapi:Expand()
		end
	end)
	windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
...
function mainapi:CreateCategoryList(categorysettings)
	...
	children.Visible = false
	children.ScrollBarThickness = 2
	children.ScrollBarImageTransparency = 0.1
	children.CanvasSize = UDim2.new()
	children.Parent = window
	...
	window.MouseButton1Click:Connect(function()
		if inputService:GetMouseLocation().Y < window.AbsolutePosition.Y + 45 then
			categoryapi:Expand()
		end
	end)
	windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
...
-- Camo Print will be applied to individual panels via function below

local scarcitybanner = Instance.new('TextLabel')
scarcitybanner.Size = UDim2.fromScale(1, 0.02)
scarcitybanner.Position = UDim2.fromScale(0, 0.97)
scarcitybanner.BackgroundTransparency = 1
scarcitybanner.Text = 'Thank you for choosing Motion V4! join https://discord.gg/99c49d34 or click the discord button to join.'
...
mainapi.BackgroundTransparency = guipane:CreateSlider({
	Name = 'Background transparency',
	Min = 0,
	Max = 100,
	Default = 67,
	Tooltip = 'Adjust the transparency of the camo background',
	Suffix = '%',
	Function = function(value)
		for _, cat in mainapi.Categories do
			if cat.Object and cat.Object:FindFirstChild('CamoOverlay') then
				cat.Object.CamoOverlay.ImageTransparency = value / 100
			end
		end
	end
})
...
guipane:CreateToggle({
	...
	Default = true,
	Tooltip = 'Shows tooltips when hovering over options'
})
guipane:CreateToggle({
...
function mainapi:UpdateGUI(hue, sat, val, default)
	...
	for i, v in mainapi.Categories do
		if i == 'Main' then
			local v4logo = v.Object.VapeLogo:FindFirstChild('V4Logo')
			if v4logo then
				v4logo.TextColor3 = Color3.fromHSV(hue, sat, val)
			end
	...
	for _, button in mainapi.Modules do
		...
		for _, v in button.Tags do
			...
			v.BackgroundTransparency = button.Enabled and 0.2 or 1
...
if shared.maincat then
	-- Discord redirect disabled
	playersService:Kick('Your script is outdated, Get new one at https://discord.gg/99c49d34')
	return
end
